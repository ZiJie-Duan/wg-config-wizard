...
class Node:
    ...
    # Server-specific settings for wg-quick
    interface: Optional[str] = None  # External network interface (e.g., eth0)
    forward: bool = True  # Enable IP forwarding
    nat: bool = True  # Enable NAT/MASQUERADE
...
class ConfigGenerator:
    ...
    def process_nodes(self) -> None:
        ...
        self.server = Node(
            ...
            interface=server_config.get("interface"),
            forward=server_config.get("forward", True),
            nat=server_config.get("nat", True)
        )
    ...
    def _generate_server_conf(self) -> str:
        ...
        lines = [
            ...
            f"# Quick Start Guide:",
            f"# -------------------",
            f"# 1. Copy this file to the server:",
            f"#    sudo cp {self.server.name}.conf /etc/wireguard/wg0.conf",
            f"#",
            f"# 2. Set correct permissions:",
            f"#    sudo chmod 600 /etc/wireguard/wg0.conf",
            f"#",
            f"# 3. Start WireGuard:",
            f"#    sudo wg-quick up wg0",
            f"#",
            f"# 4. Enable auto-start on boot:",
            f"#    sudo systemctl enable wg-quick@wg0",
            f"#",
            f"# Useful Commands:",
            f"# ----------------",
            f"# - Check status:    sudo wg show",
            f"# - Stop WireGuard:  sudo wg-quick down wg0",
            f"# - View logs:       sudo journalctl -u wg-quick@wg0",
            f"#",
            f"# =====================================================",
            "",
            "[Interface]",
        ...
        # Add PostUp/PostDown for wg-quick
        if self.server.interface:
            ...
            if self.server.forward:
                post_up_parts.append("sysctl -w net.ipv4.ip_forward=1")

            # NAT/MASQUERADE
            if self.server.nat:
                iface = self.server.interface
                post_up_parts.append(f"iptables -A FORWARD -i %i -j ACCEPT")
                post_up_parts.append(f"iptables -A FORWARD -o %i -j ACCEPT")
                post_up_parts.append(f"iptables -t nat -A POSTROUTING -o {iface} -j MASQUERADE")
                post_down_parts.append(f"iptables -D FORWARD -i %i -j ACCEPT")
                post_down_parts.append(f"iptables -D FORWARD -o %i -j ACCEPT")
                post_down_parts.append(f"iptables -t nat -D POSTROUTING -o {iface} -j MASQUERADE")

            if post_up_parts:
                lines.append(f"PostUp = {'; '.join(post_up_parts)}")
            if post_down_parts:
                lines.append(f"PostDown = {'; '.join(post_down_parts)}")
    ...
    def _generate_client_conf(self, client: Node) -> str:
        ...
        lines = [
            ...
            f"#",
            f"# Linux:",
            f"#   1. Install WireGuard:",
            f"#      Ubuntu/Debian: sudo apt install wireguard",
            f"#      Fedora:        sudo dnf install wireguard-tools",
            f"#      Arch:          sudo pacman -S wireguard-tools",
            f"#",
            f"#   2. Copy this file:",
            f"#      sudo cp {client.name}.conf /etc/wireguard/wg0.conf",
            f"#",
            f"#   3. Set correct permissions:",
            f"#      sudo chmod 600 /etc/wireguard/wg0.conf",
            f"#",
            f"#   4. Start WireGuard:",
            f"#      sudo wg-quick up wg0",
            f"#",
            f"#   5. Enable auto-start on boot:",
            f"#      sudo systemctl enable wg-quick@wg0",
            f"#",
            f"# macOS:",
            f"#   1. Install WireGuard from App Store or: brew install wireguard-tools",
            f"#   2. Import this file in WireGuard app, or:",
            f"#      sudo cp {client.name}.conf /usr/local/etc/wireguard/wg0.conf",
            f"#      sudo wg-quick up wg0",
            f"#",
            f"# Windows:",
            f"#   1. Download WireGuard from https://www.wireguard.com/install/",
            f"#   2. Open WireGuard app -> Import tunnel(s) from file",
            f"#   3. Select this .conf file and activate",
            f"#",
            f"# iOS / Android:",
            f"#   1. Install WireGuard app from App Store / Play Store",
            f"#   2. Tap '+' -> Create from QR code or Import from file",
            f"#",
            f"# Useful Commands (Linux/macOS):",
            f"# ------------------------------",
            f"# - Check status:    sudo wg show",
            f"# - Stop WireGuard:  sudo wg-quick down wg0",
            f"# - View logs:       sudo journalctl -u wg-quick@wg0",
